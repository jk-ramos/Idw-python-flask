...
def init_app(app):
    ...
    def buscar_livro():
        ...
        if request.method == 'POST':
            ...
            if termo_busca:
                # Codificar adequadamente a URL
                termo_codificado = urllib.parse.quote(termo_busca)
                
                # URL da API Open Library para busca por título
                url = f'https://openlibrary.org/search.json?title={termo_codificado}&limit=10'
                
                try:
                    # Adicionar user-agent básico
                    ...
                    for book in search_results.get('docs', []):
                        try:
                            ...
                            # Pula livros com dados corrompidos
                            continue
                    
                    resultado = livros_encontrados[:8]  
                    
                except Exception as e:
                    resultado = f'Erro na busca: Não foi possível conectar com a API'
            else:
                resultado = "Digite um termo de busca válido"
